...
def procedure_archiving(procedure_number):
    ...
    for data in requests_data:
        archive_location_dir_parts = [requests_data, str(data[1]), str(data[0]), str(data[2])]
        # uri, links_list, name
    ...
    # обрабатываем разъяснения
    chdir(work_procedures_dir)
    with cn.open():
        explanations_data = cn.execute_query(get_explanations_data_query % vars())
    explanations_data = found_location_dir(explanations_data, 2)

    # находим для каждого файла составляющие пути, куда будем класть данный файл в архив
    # собираем их в лист allData
    for data in explanations_data:
        archive_location_dir_parts = [explanations_dir]
        # uri, links_list, name
        explanation_data = [data[2], archive_location_dir_parts, str(data[1])]
        all_data.append(explanation_data)

    # обрабатываем фичи
    chdir(work_procedures_dir)
    with cn.open():
        features_data = cn.execute_query(get_features_data_query % vars())
    features_data = found_location_dir(features_data, 3)

    # находим для каждого файла составляющие пути, куда будем класть данный файл в архив
    # собираем их в лист allData
    for data in features_data:
        archive_location_dir_parts = [requests_dir, str(data[1]), features_dir]
        # uri, links_list, name
        feature_data = [data[3], archive_location_dir_parts, str(data[2])]
    ...
    if namespace.organisation:
        ...
        for data in organisation_data:
            archive_location_dir_parts = [requests_dir, str(data[2]), organisation_data_dir, str(data[1])]
            # uri, links_list, name
